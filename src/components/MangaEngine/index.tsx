...
export function MangaEngine({
    ...
    return (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-8 mb-12">
            ...
            {/* Subject Summary */}
            <div className="bg-white border-4 border-slate-900 rounded-[2.5rem] p-10 relative overflow-hidden shadow-[12px_12px_0_rgba(15,23,42,0.1)] mx-2">
                <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        <span className="w-6 h-1 bg-slate-900 rounded-full" />
                        Anchor Subject
                    </div>
                    <p className="text-xl font-black text-slate-900 leading-tight italic">
                        "{mangaSubject}"
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {panels.map((panel, idx) => (
                    ...
                    >
                        <div className="bg-white border-2 border-slate-100 rounded-[2.5rem] p-8 h-full flex flex-col shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-500 border-b-8 border-r-8 border-slate-900/5 hover:border-slate-900/20">
                            {/* Universe / Pose Badge */}
                            ...
                            <div className="space-y-6 flex-1 flex flex-col font-sans">
                                {isModelSheet ? (
                                    <>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="space-y-2">
                                                ...
                                                <p className="text-xs font-bold text-slate-900 p-3 bg-slate-50 rounded-xl border border-slate-100">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-xs font-bold text-slate-900 leading-relaxed bg-slate-50 p-4 rounded-xl border border-slate-100 italic">
                                            {panel.art_style}
                                        </p>
                                    </div>
                                )}
                                ...
                                <div className="mt-auto pt-6 border-t border-slate-50 flex items-center gap-3">
                                    <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Outfit:</span>
                                    <span className="text-[10px] font-bold text-slate-600">{panel.outfit}</span>
                                </div>
                            </div>
                        </div>
                    </motion.div>
